...
import type { ConnectButtonProps, ConnectButtonCustomProps, ConnectButtonRenderProps } from '../types';
import { truncateAddress, formatEther, getNetworkName } from '../utils';
        ...
        <div className="relative flex-shrink-0">
          <button
            ...
            className="flex items-center 
              ...
              backdrop-blur-sm"
          >
            <Globe size={16} className="mr-2 text-blue-600 dark:text-blue-400 flex-shrink-0" />
            {chainStatus === 'full' && (
              <span className="font-medium whitespace-nowrap">{getNetworkName(chainId)}</span>
            )}
            {chainStatus === 'name' && (
              <span className="font-medium whitespace-nowrap">{getNetworkName(chainId)}</span>
            )}
            {chainStatus === 'icon' && <span className="sr-only">Network</span>}
            <ChevronDown 
              size={16} 
        ...
        {accountStatus !== 'none' && (
          <>
            ...
            {accountStatus === 'address' && (
              <span className="font-mono text-sm text-gray-700 dark:text-gray-200 truncate">
                {truncateAddress(address!)}
              </span>
            )}
            {accountStatus === 'avatar' && (
              <div className="w-6 h-6 bg-gradient-to-r from-blue-500 to-purple-500 rounded-full flex items-center justify-center text-white text-xs font-bold shadow-md flex-shrink-0">
...
// Custom子组件
const ConnectButtonCustom: React.FC<ConnectButtonCustomProps> = ({
  children,
  className = '',
}) => {
  ...
  // 获取当前链信息
  const currentChain = useMemo(() => {
    return walletContext.chains.find(chain => chain.id === walletContext.chainId) || null;
  }, [walletContext.chains, walletContext.chainId]);
  ...
  useEffect(() => {
    const fetchBalance = async () => {
      if (walletContext.isConnected && walletContext.address && walletContext.provider) {
        try {
          const balanceBigInt = await walletContext.provider.getBalance(walletContext.address);
          const formattedBalance = formatEther(balanceBigInt);
          setBalance(formattedBalance);
        } catch (error) {
          console.error('Failed to fetch balance:', error);
  ...
  const renderProps: ConnectButtonRenderProps = {
    ...
    ensName: walletContext.ensName,
    balance,
    
    // 网络信息
    chainId: walletContext.chainId,
    currentChain,
    chains: walletContext.chains,
    
    // 操作方法
    connect: walletContext.openModal,
